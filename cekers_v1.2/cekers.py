...
def detect_vulnerabilities(html_content):
    ...
    if check_xss_vulnerability(html_content):
        vulnerabilities.append((f"{Fore.YELLOW}[]{Fore.GREEN}Kerentanan XSS", f"{Fore.RED}Payload eksploitasi: alert('Kerentanan XSS terdeteksi!')"))
    if check_sql_injection_vulnerability(html_content):
        vulnerabilities.append((f"{Fore.YELLOW}[]{Fore.GREEN}Kerentanan SQL Injection", f"{Fore.RED}Payload eksploitasi: SELECT * FROM users"))
    if check_csrf_vulnerability(html_content):
        vulnerabilities.append((f"{Fore.YELLOW}[]{Fore.GREEN}Kerentanan CSRF", f"{Fore.RED}Payload eksploitasi: POST /delete_account"))
    if check_path_traversal_vulnerability(html_content):
        vulnerabilities.append((f"{Fore.YELLOW}[]{Fore.GREEN}Kerentanan Path Traversal", f"{Fore.RED}Payload eksploitasi: ../../../../etc/passwd"))
    if check_ddos_vulnerability(html_content):
        vulnerabilities.append((f"{Fore.YELLOW}[]{Fore.GREEN}Kerentanan DDoS", f"{Fore.RED}Indikasi serangan DDoS terdeteksi"))
    ...
    if sensitive_info:
        ...
        vulnerabilities.append((f"{Fore.YELLOW}[]{Fore.GREEN}Informasi Sensitif Terbocor", info_message))

    # Deteksi Pemindaian yang Tidak Sah
    if detect_illegal_scanning(html_content):
        vulnerabilities.append((f"{Fore.GREEN}[SUCCES]{Fore.RED}Pemindaian Tidak Sah", "Illegal scanning detected!"))
...
def save_html_to_file(html_content):
    try:
        ...
        logger.info(f"{Fore.GREEN}[SUCCES]Kode HTML disimpan dalam file index.html")
    except Exception as e:
        logger.error(f"{Fore.RED}Gagal menyimpan file: {e}")
...
def display_html_code(url):
    try:
        ...
        logger.info(f"{Fore.GREEN}[SUCCES]Kode HTML dari {url} disimpan dalam file index.html")
    except requests.RequestException as e:
        logger.error(f"{Fore.RED}Gagal memuat halaman: {e}")
    except Exception as e:
        logger.error(f"{Fore.RED}Terjadi kesalahan: {e}")
...
def main():
    banner_text = "CEKERS"
    versi_beta = "Beta Version 1.2 BY:TRS"
    ...
    target_url = input(f"{Fore.GREEN}[URL] >{Fore.YELLOW}: ")
    ...
    try:
        ...
        print(f"{Fore.RED}[FAILED]Gagal memuat halaman: {e}{Style.RESET_ALL}")
        return
    ...
    if server_header:
        print(f"{Fore.YELLOW}[]{Fore.GREEN}Informasi Server:{Style.RESET_ALL} {Fore.YELLOW}{server_header}")
    else:
        print(f"{Fore.RED}[FAILED]{Fore.YELLOW}[FAILED]Header Server tidak ditemukan dalam respons HTTP.{Style.RESET_ALL}")

    # Memeriksa alamat IP atau nama host server
    try:
        hostname = target_url.split('//')[1].split('/')[0]  # Mengambil bagian hostname dari URL
        ip_address = socket.gethostbyname(hostname)
        print(f"{Fore.YELLOW}[]{Fore.GREEN}Alamat IP dari server:{Style.RESET_ALL} {Fore.YELLOW}{ip_address}")
        ...
        if open_ports:
            print(f"{Fore.YELLOW}[]{Fore.GREEN}Port terbuka pada server:{Style.RESET_ALL} {Fore.YELLOW}{', '.join(map(str, open_ports))}")
        else:
            print(f"{Fore.RED}[FAILED]{Fore.YELLOW}Tidak ada port terbuka yang ditemukan.{Style.RESET_ALL}")
        ...
        print(f"{Fore.YELLOW}[]{Fore.GREEN}Lokasi server:{Style.RESET_ALL} {Fore.YELLOW}{google_maps_link}")
    except Exception as e:
        print(f"{Fore.RED}[FAILED]{Fore.RED}Gagal mendapatkan informasi lokasi server: {e}{Style.RESET_ALL}")

    if check_wordpress(html_content):
        print(f"{Fore.GREEN}[SUCCES]{Fore.BLUE}Situs web merupakan situs web WordPress.{Style.RESET_ALL}")
    else:
        print(f"{Fore.RED}[FAILED]{Fore.RED}Situs web bukan merupakan situs web WordPress.{Style.RESET_ALL}")

    sensitive_info = detect_sensitive_information(html_content)
    if sensitive_info:
        print(f"{Fore.GREEN}[SUCCES]{Fore.YELLOW}Ditemukan informasi sensitif:{Style.RESET_ALL}")
        for info in sensitive_info:
            print(f"{Fore.YELLOW}{info}{Style.RESET_ALL}")

    if detect_illegal_scanning(html_content):
        print(f"{Fore.GREEN}[SUCCES]{Fore.YELLOW}Pemindaian tidak sah terdeteksi.{Style.RESET_ALL}")

    vulnerabilities = detect_vulnerabilities(html_content)
    if vulnerabilities:
        print(f"{Fore.GREEN}[SUCCES]{Fore.YELLOW}Ditemukan kerentanan:{Style.RESET_ALL}")
        for vulnerability_name, vulnerability_description in vulnerabilities:
            print(f"{Fore.RED}{vulnerability_name}:{Style.RESET_ALL} {vulnerability_description}")
    else:
        print(f"{Fore.RED}[FAILED]{Fore.RED}Tidak ditemukan kerentanan.{Style.RESET_ALL}")
    ...
    try:
        ...
        if 'set-cookie' in response.headers:
            ...
            if cookies:
                ...
                print(f"{Fore.GREEN}[SUCCES]{Fore.GREEN}Cookie telah berhasil dicuri dan disimpan dalam file 'cookies.pkl'.{Style.RESET_ALL}")
            else:
                print(f"{Fore.YELLOW}[TIRF]{Fore.RED}Tidak ada cookie yang ditemukan dalam respons HTTP.{Style.RESET_ALL}")
        else:
            print(f"{Fore.RED}[FAILED]{Fore.RED}Respons HTTP tidak mengandung cookie.{Style.RESET_ALL}")

    except requests.exceptions.RequestException as e:
        print(f"{Fore.RED}[FAILED]{Fore.RED}Gagal melakukan permintaan HTTP: {e}{Style.RESET_ALL}")
    except Exception as e:
        print(f"{Fore.RED}[FAILED]{Fore.RED}Terjadi kesalahan: {e}{Style.RESET_ALL}")

cookies = read_cookies_from_file("cookies.pkl")
if cookies:
    print(f"{Fore.GREEN}[SUCCES]Cookies berhasil dibaca:{Style.RESET_ALL}")
    print(cookies)
else:
    print(f"{Fore.RED}[FAILED]{Fore.RED}Gagal membaca cookies dari file.{Style.RESET_ALL}")
...
def extract_contact_info_from_url(target_url):
    try:
        ...
        logger.error(f"{Fore.RED}[FAILED]{Fore.RED}Gagal memuat halaman: {e}{Style.RESET_ALL}")
        return [], []
    except Exception as e:
        logger.error(f"{Fore.RED}[FAILED]{Fore.RED}Terjadi kesalahan: {e}{Style.RESET_ALL}")
        return [], []
...
def display_contact_info_from_url(target_url):
    ...
    if phone_numbers:
        logger.info(f"{Fore.RED}[FAILED]{Fore.RED}Nomor telepon yang ditemukan:{Style.RESET_ALL}")
        for phone_number in phone_numbers:
            logger.info(phone_number)
    else:
        logger.info(f"{Fore.RED}[FAILED]{Fore.RED}Nomor telepon tidak ditemukan.{Style.RESET_ALL}")
    
    if emails:
        logger.info(f"{Fore.RED}[FAILED]{Fore.RED}Alamat email yang ditemukan:{Style.RESET_ALL}")
        for email in emails:
            logger.info(email)
    else:
        logger.info(f"{Fore.RED}[FAILED]{Fore.RED}Alamat email tidak ditemukan.{Style.RESET_ALL}")
