...
def main():
    banner_text = "CEKERS"
    versi_beta = "Beta Version 1.0 BY:TRS"
    ...
    target_url = input(f"{Fore.YELLOW}Masukkan URL situs web yang ingin dipindai: ")
    ...
    if check_wordpress(html_content):
        print(f"{Fore.BLUE}Situs web merupakan situs web WordPress.{Style.RESET_ALL}")
    else:
        print(f"{Fore.RED}Situs web bukan merupakan situs web WordPress.{Style.RESET_ALL}")
    ...
    if vulnerabilities:
        ...
        print(f"{Fore.RED}Tidak ditemukan kerentanan.{Style.RESET_ALL}")
    ...
    try:
        ...
        if 'set-cookie' in response.headers:
            ...
            if cookies:
                ...
                print(f"{Fore.GREEN}Cookie telah berhasil dicuri dan disimpan dalam file 'cookies.pkl'.{Style.RESET_ALL}")
            else:
                print(f"{Fore.RED}Tidak ada cookie yang ditemukan dalam respons HTTP.{Style.RESET_ALL}")
        else:
            print(f"Respons HTTP tidak mengandung cookie.{Style.RESET_ALL}")

    except requests.exceptions.RequestException as e:
        print(f"Gagal melakukan permintaan HTTP: {e}{Style.RESET_ALL}")
    except Exception as e:
        print(f"Terjadi kesalahan: {e}{Style.RESET_ALL}")

cookies = read_cookies_from_file("cookies.pkl")
if cookies:
    print(f"Cookies berhasil dibaca:{Style.RESET_ALL}")
    print(cookies)
else:
    print(f"Gagal membaca cookies dari file.{Style.RESET_ALL}")
...
def extract_contact_info_from_url(target_url):
    try:
        ...
        logger.error(f"Gagal memuat halaman: {e}{Style.RESET_ALL}")
        return [], []
    except Exception as e:
        logger.error(f"Terjadi kesalahan: {e}{Style.RESET_ALL}")
        return [], []
...
def display_contact_info_from_url(target_url):
    ...
    if phone_numbers:
        logger.info(f"{Fore.RED}Nomor telepon yang ditemukan:{Style.RESET_ALL}")
        for phone_number in phone_numbers:
            logger.info(phone_number)
    else:
        logger.info(f"{Fore.RED}Nomor telepon tidak ditemukan.{Style.RESET_ALL}")
    
    if emails:
        logger.info(f"{Fore.RED}Alamat email yang ditemukan:{Style.RESET_ALL}")
        for email in emails:
            logger.info(email)
    else:
        logger.info(f"{Fore.RED}Alamat email tidak ditemukan.{Style.RESET_ALL}")
